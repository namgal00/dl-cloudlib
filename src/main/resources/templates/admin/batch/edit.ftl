...
<html>
	...
	<body>
		<article class="main">
			...
			<div class="nav">修改批次</div>
			<div class="pubFormTable">
				<form action="${base}/admin/batch/edit" method="post" id="formId" >
					<input type="hidden" name="id" value="${batch.id}"  />
					<table cellpadding="0" cellspacing="0" border="0">
						<tr>
							<td>批次名称:</td>
							<td><input type="text" name="name" data-oldname="${batch.name}"  value="${batch.name}" onfocus="cleanNotice(this)" onblur="checkName(this);" placeholder="请输入名称" id="name" /><span id="name_notice" class="field_notice">*</span></td>
						</tr>

						<tr>
							<td>描述:</td>
							<td><textarea name="descript" id="description">${batch.descript}</textarea>
							</td>
						</tr>
						<tr>
							<td></td>
